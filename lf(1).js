...
//				};
			...
			$.log.info("send[" + action + "][" + options.data.tokenId + "]:" + $.util.jsonToStr(options.data));
			var success = options.success;
			var error = options.error;
			var _success = function(data, textStatus, xhr) {
				$.log.info("received[" + action + "]：" + JSON.stringify(data));
...
//			data.uid = ""
//			data.auth_sign = REQUESTDATA.auth_sign
            var tempData = {
...
//      }
        ...
        $.log.info('config.data:', config.data)
        $.log.info('send[' + config.url + ']:' + $.util.jsonToStr(config.data))
        config.data = {app:1};
        return config;
    }, function(error) {
    ...
    axios.interceptors.response.use(function(response) {
        ...
        console.log('发送之后：',response)
        return response.data;
    }, function(error) {
        // Do something with response error
        console.error('发送之后：',error.code)
        console.error('发送之后：',error.message)
        var obj = {}
